...
## وابستگی خاص Events.Outbox به Onion

package `Zamin.Extensions.Events.Outbox` در وضعیت فعلی برای کارکرد صحیح خود به اجزایی از معماری Onion وابسته است. این وابستگی به دلیل نیاز به خواندن eventها از Aggregate Rootها و ثبت آن‌ها در ساختار Outbox شکل گرفته است.

به همین دلیل، این package را نمی‌توان در وضعیت فعلی یک extension کاملاً مستقل در نظر گرفت و مهاجرت آن باید همراه با بخش‌های مرتبط در Onion انجام شود.
