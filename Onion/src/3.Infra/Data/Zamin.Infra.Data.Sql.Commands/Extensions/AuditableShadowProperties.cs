...
public static class AuditableShadowProperties
{
	...
	public static void AddAuditableShadowProperties<TId>(this ModelBuilder modelBuilder)
	{
		foreach (var entityType in modelBuilder.Model
											   .GetEntityTypes().Where(c => typeof(Entity<TId>).IsAssignableFrom(c.ClrType)))
		{
	...
	public static void SetAuditableEntityPropertyValues<TId>(
		...
		IUserInfoService userInfoService)
	{
		...
		var modifiedEntries = changeTracker.Entries<Entity<TId>>().Where(x => x.State == EntityState.Modified);
		...
		var addedEntries = changeTracker.Entries<Entity<TId>>().Where(x => x.State == EntityState.Added);
